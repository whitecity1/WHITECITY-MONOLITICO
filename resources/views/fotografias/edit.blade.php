...
<div class="block mx-auto my-12 p-8 bg-white w-1/2 border border-gray-200 rounded-lg shadow-lg">
    ...
    <h1 class="text-3xl text-center pt-24 font-bold">EDITAR REGISTRO DE FOTOGRAFÍAS</h1>

    <form class="mt-4"  action="/fotografias/{{$fot->id}}" method="POST" enctype="multipart/form-data">
   
        @csrf
        @method('PUT')
        Nombre:
        <input type="text" class="border border-gray-200 rounded-md bg-gray-200 w-full text-lg placeholder-gray-900 p-2 my-2 focus:bg-white" placeholder="Descripción"
        id="nombre" name="nombre" value="{{$fot->nombre}}">
        ...
        Descripción:
        <input type="text" class="border border-gray-200 rounded-md bg-gray-200 w-full text-lg placeholder-gray-900 p-2 my-2 focus:bg-white" placeholder="Descripción"
        id="descripcion" name="descripcion" value="{{$fot->descripcion}}">
